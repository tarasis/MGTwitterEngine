...
- (NSString *) getStatusesForList:(NSString *)listname byUser:(NSString *)username withOptions:(NSDictionary *)options
{
	...
	NSString * sinceID = [options objectForKey:@"since_id"]; 
	NSString * maxID = [options objectForKey:@"max_id"];
	NSString * perPage = [options objectForKey:@"per_page"];
	NSString * page = [options objectForKey:@"page"];
    ...
    return [self _sendRequestWithMethod:nil path:path queryParameters:params body:body 
                            requestType:MGTwitterUserListStatusesRequest 
                           responseType:MGTwitterStatus];
}
...
- (NSString *) deleteUser:(NSString *)user fromList:(NSString *)list
{
	if (!user || !list) {
		return nil;
	}
	...
	NSString *path = [NSString stringWithFormat:@"%@/%@/members.%@", [self username], list, API_FORMAT];
	NSDictionary * params = [NSDictionary dictionaryWithObjectsAndKeys:user, @"id", @"DELETE", @"_method", nil];
	
	NSString *body = [self _queryStringWithBase:nil parameters:params prefixed:NO];
	
    return [self _sendRequestWithMethod:HTTP_POST_METHOD path:path queryParameters:params body:body
                            requestType:MGTwitterUserListMembershipDeleteRequest
                           responseType:MGTwitterUserLists];
}
...
- (NSString *) getListMembersForList:(NSString *)list ownedBy:(NSString *)user withOptions:(NSDictionary *)options
{
	...
	NSString *path = [NSString stringWithFormat:@"%@/%@/members.%@", user, list, API_FORMAT];
	
    NSMutableDictionary *params = [NSMutableDictionary dictionaryWithCapacity:0];
	
	NSString * cursor = [options objectForKey:@"cursor"];
	NSString * includeEntities = [options objectForKey:@"include_entities"];
    ...
    return [self _sendRequestWithMethod:nil path:path queryParameters:params body:body 
                            requestType:MGTwitterUserListMembershipListRequest 
                           responseType:MGTwitterUser];
}
...
- (NSString *) isUser:(NSString *)user aMemberOfList :(NSString *)list ownedBy:(NSString *) owner withOptions:(NSDictionary *)options
{
    ...
    return [self _sendRequestWithMethod:nil path:path queryParameters:params body:body
                            requestType:MGTwitterUserListMembershipCheckRequest
                           responseType:MGTwitterUser];
...
- (NSString *) getSubscribersOfList:(NSString *)list ownedBy:(NSString *)user withOptions:(NSDictionary *)options
{
	...
	NSString *path = [NSString stringWithFormat:@"%@/%@/subscribers.%@", user, list, API_FORMAT];
    NSMutableDictionary *params = [NSMutableDictionary dictionaryWithCapacity:0];
    ...
    return [self _sendRequestWithMethod:nil path:path queryParameters:params body:body
                            requestType:MGTwitterUserListSubscribersRequest
                           responseType:MGTwitterUser];
}
